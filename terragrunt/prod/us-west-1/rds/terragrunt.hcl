...
locals {
  ...
  cluster_name = "aws-rds-prod-usw1"
}
...
inputs = merge(
  local.rds_common.locals.rds_common_inputs,
  {
    ...
    # DR region joins the global cluster created in the primary region.
    create_global_cluster     = false
    global_cluster_identifier = "aws-rds-prod-use2-global"
